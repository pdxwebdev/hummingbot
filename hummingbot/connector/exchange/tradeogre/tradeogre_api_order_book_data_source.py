...
class TradeogreAPIOrderBookDataSource(OrderBookTrackerDataSource):
    ...
    async def _order_book_snapshot(self, trading_pair: str) -> OrderBookMessage:
        ...
        snapshot_msg: OrderBookMessage = TradeogreOrderBook.snapshot_message_from_exchange(
            snapshot,
            snapshot_timestamp,
    ...
    async def _parse_trade_message(self, raw_message: Dict[str, Any], message_queue: asyncio.Queue):
        if "info" not in raw_message:
            print(raw_message)
            trading_pair = await self._connector.trading_pair_associated_to_exchange_symbol(symbol=raw_message["t"])
    ...
    async def _parse_order_book_diff_message(self, raw_message: Dict[str, Any], message_queue: asyncio.Queue):
        if "info" not in raw_message:
            trading_pair = await self._connector.trading_pair_associated_to_exchange_symbol(symbol=raw_message["t"])
            order_book_message: OrderBookMessage = TradeogreOrderBook.diff_message_from_exchange(
                raw_message, time.time(), {"trading_pair": trading_pair})
            message_queue.put_nowait(order_book_message)

    def _channel_originating_message(self, event_message: Dict[str, Any]) -> str:
        channel = ""
        if "info" not in event_message:
